		...
		<div class="SheetEditorMenuContainer"
			...
			>
				<button
					...
					use:tooltip={{ text: 'Editmode, allows in View Edits', type:'verbose' }} 
					>{"Stop Edit	"}</button
				>
				<button
					data-active={$editLayout_01}
					on:click={() =>{editWasClicked.set(true);editLayout_01.set(!$editLayout_01)}}
					use:tooltip={{ text: 'Create Rows that stack vertically<br> you can drag and drop them to switch places of Rows', type:'verbose' }} 
					>{"Layout Row	"}</button
				>
				<button
					data-active={$editLayout_02}
					on:click={() =>{editWasClicked.set(true);editLayout_02.set(!$editLayout_02)}}
					use:tooltip={{ text: 'Create Columns in your rows<br> Columns stack horizontally<br> you can drag and drop them to switch places', type:'verbose' }} 
					>{"Layout Col	"}</button
				>
				<button
					data-active={$editLayout_03}
					on:click={() =>{editWasClicked.set(true);editLayout_03.set(!$editLayout_03)}}
					use:tooltip={{ text: 'Create items in your Columns<br>Items Stack vertically in a Column<br> when you drag an item you change what Column they belong to<br> to change vertical order use buttons', type:'verbose' }} 
					>{"Layout Items"}</button
				>
			</div>
		</div>
